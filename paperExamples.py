...
"""
To run the examples presented in the publication titled 
" MMTOuNN : Multi-Material Topology Optimization using Neural Networks "
"""


from neuralTO_MM import *
import numpy as np
...
if(example == 1): # Validation examples
    ...
    numNeuronsPerLyr = 25; # the height of the NN
    ...
    # [1,2,3,4]
    EMaterials =  np.array([2, 1., 0.6, 0.01]); # Young modulus of the materials # , 2.0
    massDensityMaterials = np.array([1., 0.6, 0.4, 0.1]); # wt (/area) of the candidate materials
    fillColors = ['0.90','black','red','blue','pink'];
    

    # [1,3,4]
    # EMaterials =  np.array([2,  0.6, 0.01]); # Young modulus of the materials # , 2.0
    # massDensityMaterials = np.array([1., 0.4, 0.1]); # wt (/area) of the candidate materials
    ...
    topOpt.initializeFE(exampleName,nelx, nely, elemArea, force, fixed, 'cpu', penal, nonDesignRegion, EMaterials);
...
if(example == 2):   # Validity michell
    ...
    # [1,2,3]
    EMaterials =  np.array([1.,0.6,0.2]); 
    massDensityMaterials = np.array([1.,0.7,0.4]); # 
    fillColors = ['0.90','black','red','blue'];
    
    # [1,2]
    EMaterials =  np.array([1.,0.6]); 
    massDensityMaterials = np.array([1.,0.7]); # 
    fillColors = ['0.90','black','red'];
    
    # [1,3]
    EMaterials =  np.array([1.,0.2]); 
    massDensityMaterials = np.array([1.,0.4]); # 
    fillColors = ['0.90','black','blue'];
    
    # [1]
    EMaterials =  np.array([1.]); # Young modulus of the materials # , 2.0
    massDensityMaterials = np.array([1.]); # 
    ...
    force[int(2*nelx*(nely+1)/4)+1 ,0]= -1; # nelx should be multiple of 4
...
if(example == 3): # convergecne 
    ...
    elemArea = 1.0; # assumes sizeX = sizeY.
    desiredMassFraction =  0.5 
    EMaterials =   np.array([380, 210, 110]); # Young modulus of the materials # , 2.0 np.array([3.]) #
    massDensityMaterials =  np.array([19250, 7800, 4390]); # wt (/area np.array([1.]); #
...
if(example == 4):   # comp cost michell
    ...
    maxNumMat = 11;
    fillColors = ['0.90','red','cyan','black','orange','blue','orange','green','yellow','magenta', 'navy'];
    ...
    for numMat in range(1, maxNumMat):
        for dvc in devices:
            ...
            start = time.perf_counter();
            topOpt = TopologyOptimizer();
            topOpt.initializeFE(exampleName,nelx, nely, elemArea, force, fixed, dvc, penal, nonDesignRegion, EMaterials);
            topOpt.initializeOptimizer(numLayers, numNeuronsPerLyr, desiredMassFraction, massDensityMaterials, symXAxis, symYAxis); 
            lossHist = topOpt.train(maxEpochs, minEpochs, useSavedNet);
            print("Time taken: {:.2F}".format(time.perf_counter() - start));
...
if(example == 5): # Neural net arch dependency 
    ...
    penal = 2.;
    useSavedNet = False;
    numLayers = [4,5,7,8,9,10]; # the depth of the NN
    numNeuronsPerLyr =[12,15,25,30,35,50]; # the height of the NN
    ...
    for i in range(len(numLayers)):
        start = time.perf_counter();
        topOpt = TopologyOptimizer();
        topOpt.initializeFE(exampleName,nelx, nely, elemArea, force, fixed, 'gpu', penal, nonDesignRegion, EMaterials);
        topOpt.initializeOptimizer(numLayers[i], numNeuronsPerLyr[i], desiredMassFraction, massDensityMaterials, symXAxis, symYAxis);
        lossHist = topOpt.train(maxEpochs, minEpochs, useSavedNet);
        print("Time taken: {:.2F}".format(time.perf_counter() - start));
        topOpt.plotMaterialImage(resolution = 15, grids = False, fillColors = ['0.90','red','cyan','black'])
        modelWeights, modelBiases = topOpt.topNet.getWeights();
...
if(example == 6):   # mesh dependence michell
    ...
    exampleName = 'Michell'
    for i in range(len(nelx)):
        ndof = 2*(nelx[i]+1)*(nely[i]+1);
        elemArea = lengthX*lengthY/(nelx[i]*nely[i]); 
        force = np.zeros((ndof,1))
        dofs=np.arange(ndof);
        fixed=np.array([ 0,1,2*(nelx[i]+1)*(nely[i]+1)-2*nely[i]+1] );
        force[int(2*nelx[i]*(nely[i]+1)/4)+1 ,0]= -1; # nelx should be multiple of 4
        force[int(2*nelx[i]*(nely[i]+1)/2)+1 ,0]= -2;
        force[int(2*nelx[i]*(nely[i]+1)*3/4)+1 ,0]= -1;
        nonDesignRegion = None;
        symXAxis = False;
        symYAxis = True;
        ...
        start = time.perf_counter();
        topOpt = TopologyOptimizer();
        topOpt.initializeFE(exampleName,nelx[i], nely[i], elemArea, force, fixed, 'cpu', penal, nonDesignRegion, EMaterials);
        topOpt.initializeOptimizer(numLayers, numNeuronsPerLyr, desiredMassFraction, massDensityMaterials, symXAxis, symYAxis);
...
if(example == 7): # Pareto w and w/o restart 
    ...
    nely = 30
    elemArea = 1.0; # assumes sizeX = sizeY.
    desiredMassFraction =  [1., 0.9, 0.7, 0.5, 0.3, 0.2];
    EMaterials =   np.array([3, 2, 1]); # Young modulus of the materials # , 2.0 np.array([3.]) #
    massDensityMaterials =  np.array([1, 0.7, 0.4]); # wt (/area np.array([1.]); #
    ...
    for mfrac in desiredMassFraction:
        device = 'cpu';
        start = time.perf_counter();
        topOpt = TopologyOptimizer();
        topOpt.initializeFE(exampleName,nelx, nely, elemArea, force, fixed, device, penal, nonDesignRegion, EMaterials);
        topOpt.initializeOptimizer(numLayers, numNeuronsPerLyr, mfrac, massDensityMaterials, symXAxis, symYAxis);
        lossHist = topOpt.train(maxEpochs, minEpochs, useSavedNet);
        print("Time taken: {:.2F}".format(time.perf_counter() - start));
        # topOpt.plotConvergence();
...
if(example == 8): # Boundary extraction
    ...
    EMaterials =  np.array([380, 210, 110]); # Young modulus of the materials # , 2.0
    massDensityMaterials = np.array([19250, 7800, 4390]); # wt (/area
    fillColors = ['0.90','red','cyan','black'];
    exampleName = 'Michell'
    ndof = 2*(nelx+1)*(nely+1);
    elemArea = 1.0;
